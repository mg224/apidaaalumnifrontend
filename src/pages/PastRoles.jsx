...
export default function PastRoles() {
  ...
  const handleSearch = (e) => {
    e.preventDefault()
    if (searchQuery.trim()) {
      navigate(`/data?pastRoles=${encodeURIComponent(searchQuery.trim())}`)
    }
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 px-30 py-40 flex justify-center">
        <div className="w-full max-w-xl space-y-8">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6 text-center">Search Alumni by Past Roles</h1>
  
          <form onSubmit={handleSearch} className="w-full">
            <div className="relative w-full">
              <input 
                type="text"
                placeholder="Enter an APIDAA role"
                value={searchQuery}
